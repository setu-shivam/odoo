...
class SetuReturnOrder(models.Model):
    ...
    def onchange_picking_id(self):
        ...
        if stock_picking:
            ...
            claim_lines = [
                (0, 0, {'product_id': move.product_id.id, 'quantity': move.quantity_done, 'stock_move_id': move.id})
                for
                move in stock_picking.move_ids if move.quantity_done > 0]
    ...
    def action_return_order_send(self):
        ...
        try:
            template_id = ir_model_data.sudo().get_object_reference('setu_rma', 'mail_setu_rma_details_notification')[1]
        except ValueError:
            template_id = False
        try:
    ...
    def create_return_picking(self, claim_lines=False):
        ...
        return_picking_wizard = self.env['stock.return.picking'].with_context(
            active_id=self.return_picking_id.id if claim_lines else self.stock_picking_id.id).sudo().create(vals)
        return_lines = []
    ...
    def action_return_approve(self):
        ...
        for line in self.return_order_line_ids:
            ...
            if total_qty >= line.stock_move_id.quantity_done:
