...
export class setu_rma_dashboard extends Component{
    ...
    async fetch_data() {
        ...
        const result = await this.rpc(  '/setu_rma/fetch_dashboard_data', {
               date_from: this.date_from.year()+'-'+(this.date_from.month()+1)+'-'+this.date_from.date(),
                ...
                company_id: session.user_context.allowed_company_ids,//self.controlPanelProps.action.context.allowed_company_ids,
    ...
    _computeControlPanelProps() {
        const $searchview = $(QWeb.render("setu_rma.DateRangeButtons", {
            content: this,
        }));
        ...
        $('#setu_rma_panel').html($searchview)
        //this.ControlPanel.Props.cp_content = { $searchview };
    }
    ...
    render_dashboards() {
        var self = this;
        $('.o_rma_dashboard').append(QWeb.render('setu_rma.dashboard_content_qweb', {content: self}));
    }
