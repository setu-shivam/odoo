DROP FUNCTION IF EXISTS public.icr_get_stock_data(integer[], integer[], integer[], integer[], text, date, date);
CREATE OR REPLACE FUNCTION public.icr_get_stock_data(
    ...
    IN start_date date,
    IN end_date date)
  RETURNS TABLE(company_id integer, company_name character varying, product_id integer, product_name character varying, product_category_id integer, category_name character varying, warehouse_id integer, warehouse_name character varying, product_tmpl_id integer, product_qty numeric) AS
$BODY$
DECLARE
    source_usage text;
    dest_usage text;
...
BEGIN
    ...
    RETURN QUERY
    --Return stock move data - sum of product quantity,group by company
    Select
        T.company_id,
        T.company_name,
        T.product_id,
    ...
    --Return all stock move data
        ...
        From
            stock_move move
                ...
                Inner Join product_template tmpl on tmpl.id = prod.product_tmpl_id and tmpl.type != 'combo' and tmpl.is_storable
    ...
    group by T.company_id, T.product_id, T.category_id, T.warehouse_id, T.company_name, T.product_name, T.category_name, T.warehouse_name, T.product_tmpl_id
    ;
END; $BODY$
