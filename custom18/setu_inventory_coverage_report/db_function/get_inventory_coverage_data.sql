...
$BODY$
    ...
    BEGIN
        ...
        --If vendor strategy is cheapest
        IF vendor_strategy = 'cheapest' then
            ...
            --Prepare data to insert into report model
            select
            ...
            from
            (
            --Returns current stock and sales data group by company
            select
                cov.company_id,
                cov.company_name,
                cov.product_id,
            ...
            from
                (
                --Returns current stock and sales
                ...
                left join (select
                            *
                       from (
                            --Returns data with vendor and order by vendor price(cheapest vendor)
                            select
                            ...
                            from
                                (
                                --data with vendor details like delay,min quantity,price and price in currency
                                select *,
        ...
        --else part if vendor strategy is quickest
        else
          Insert into setu_inventory_coverage_analysis_bi_report(company_id,company_name,product_id,product_name,product_category_id,category_name,warehouse_id,warehouse_name,product_tmpl_id,current_stock,average_daily_sales,coverage_days,wizard_id,
                                                                   partner_id, currency_id, price, delay, min_qty, price_in_currency, coverage_ratio, out_stock_days, sold_qty)
            ...
            from
                ...
                left join (select
                            *
                       from (
                       --Returns data with vendor and order by vendor delay(quickest vendor)
                            select
