...
class SetuDataGenerator(models.TransientModel):
    ...
    is_allow_child_company_journal = fields.Boolean("Allow Child Company Journal", default=False)

    def get_filters(self):
        """
        Purpose: This Product is used for collect details like company,warehouse,product and product category from user
        :return:
        """
        products = []
        warehouses = []
        if self.product_category_ids:
            domain = [('categ_id', 'child_of', self.product_category_ids.ids),
        ...
        if not products:
            products = self.env['product.product'].search([('product_tmpl_id.is_storable', '=', True)]).ids
    ...
    def create_sale_order(self, partners, end_date, order_creation_loop):
        """
        Purpose : This method is used for create sale order based on user inputs
                -This method is used for set product prices
        :param partners:
        :param end_date:
        :param order_creation_loop:
        :return:
        """
        ...
        while order_count <= self.max_order_per_day:
            ...
            if not pl:
                raise ValidationError("Please set pricelist for create the sale order.")
            ...
            if so_line_vals:
                so_vals = {
                    ...
                    'is_allow_child_company_journal': self.is_allow_child_company_journal
                }
    ...
    def prepare_sale_order_line_vals(self, products, wh, scheduled_date):
        """
        Purpose: This method is used for crete vals for sale order line
                -This method is used for sum product quantity of draft sale order
                -Check product available quantity based on user inputs.
        :param products:
        :param wh:
    ...
    def generate_so(self):
        """
        Purpose : This method call when user click on generate so button from setu data generator wizard
                -This method is used for display list view of generated sale orders
        :return:
        """
        ...
        partners = self.env['res.partner'].sudo().search([('id', 'not in', exclude_partners.ids),
                                                          ('company_id', '=', False)]).ids
        sale_orders = []
        while i >= 1:
    ...
    def confirm_sale_order_company_wise(self):
        """
        Purpose : This method is used for auto confirm draft sale orders,
                validate delivery and create customer invoice
                based on delivery gap configuration
        :return:
        """
        company_ids = self.env.companies
        for company_id in company_ids:
            sale_orders = self.env['sale.order'].search(
                [('company_id', '=', company_id.id), ('state', 'in', ('draft', 'sent'))],
    ...
    def complete_sale_flow(self, sales_order):
        """
        Purpose: This method is used for confirmed picking
                and create customer invoice based on user configuration
        :param sales_order:
        :return:
        """
        delivery_gap = self.env['ir.config_parameter'].sudo().get_param('setu_data_generator.sale_delivery_gap')
        delivery_gap = int(delivery_gap)
        ...
        for order in sales_order:
            ...
            for picking in order.picking_ids:
                ...
                delivery_date = (order.date_order + timedelta(days=day)).date().strftime('%Y-%m-%d')
                so_context.update({'history_order_date': delivery_date, 'force_period_date': delivery_date})
    ...
    def create_purchase_order(self, partners, end_date, order_creation_loop):
        """
        Purpose: This method is used for create purchase order
                -This method is used for compute currency rate
        :param partners:
        :param end_date:
        :param order_creation_loop:
        :return:
        ...
        while order_count <= self.max_order_per_day:
            ...
            wh = self.env['stock.warehouse'].browse(warehouse_id)
            fpos = self.env['account.fiscal.position'].with_company(wh.company_id)._get_fiscal_position(partner)
            if po_line_vals := self._prepare_purchase_order_line_vals(wh, fpos, products, creation_date, partner,
                ...
                po_vals = {
                    ...
                    'create_date': creation_date,
                    'currency_id': wh.company_id.currency_id.id,
                    'is_allow_child_company_journal': self.is_allow_child_company_journal
                }
    ...
    def _prepare_purchase_order_line_vals(self, wh, fpos, products, scheduled_date, partner, company):
        """
        Purpose : This method is used for create purchase order line
                -This method is used for set product price
                -This method is used for check  stock of product based on user configuration
        :param wh:
        :param fpos:
        :param products:
        :param scheduled_date:
        ...
        if products := products.with_context(warehouse=wh.id).filtered(lambda x: ((x.qty_available + sum(
            ...
            while prod_count <= product_per_order:
                ...
                seller = product.seller_ids.filtered(
                    lambda s: s.partner_id.id == partner.id and s.currency_id.id == company.currency_id.id)
                price_unit = 0.0
                if seller:
    ...
    def generate_po(self):
        """
        Purpose : This method call when user click on generate po button from setu data generator wizard
                -This method is used for display list view of generated purchase orders.
        :return:
        """
        start_date = self.start_date
        end_date = self.end_date
    ...
    def complete_purchase_flow(self, purchase_order):
        """
        Purpose : This method is used for confirm picking , validate purchase order delivery
                and create vendor bill based on user configuration
                ...
                for picking in order.picking_ids:
                    ...
                    for p in moves:
                        if p.date_deadline:
                            ...
                            for l in p.move_line_ids:
                                l.write({'date': delivery_date})
            self.with_context(original_context).create_post_bills(purchases=order)
        return True

    def create_post_invoices(self, sales=False):
        """
        Purpose : This method is used for create customer invoice and register payment
        :param sales:
        :return:
        """
        if not sales:
    ...
    def create_post_bills(self, purchases=False):
        """
        Purpose : This method is used for create vendor bill and register payment
        :param purchases:
        :return:
        """
        if not purchases:
        ...
        if job_queue_data_generation:
            ...
            bill_post_days = self.env['ir.config_parameter'].sudo().get_param('setu_data_generator.bill_post_days')
    ...
    def generate_acc_entries(self):
        """
        Purpose : This method is used for generate journal entries based on user inputs
                -Also used for display list view of generated journal entries
        :return:
        """
...
class SetuAccountingEntryDataGenerator(models.TransientModel):
    ...
    account_id = fields.Many2one("account.account", string="Account", store=True,
                                 domain="[('company_ids', 'in', company_id),('account_type', 'in', ['expense', 'income'])]")
    ...
    interval_type = fields.Selection([("days", "Day"),
                                      ("months", "Month"),
                                      ("years", "Year")],
                                     default='months', string="Type")

    @api.onchange('account_id', 'company_id')
    def onchange_account_id_company_id(self):
        """
        Purpose: This method is used for search miscellaneous journals and bank account company wise
        :return:
        """
        if self.company_id:
            ...
            if account := self.env['account.account'].sudo().search(
                    [('name', '=', 'Bank'), ('company_ids', 'in', self.company_id.id)], limit=1):
                self.reconcile_account_id = account.id

    @api.depends("account_id", "company_id")
    def _get_domain(self):
        """
        Purpose: This method is used for search company wise journals
                -Also used for search company wise Bank and Cash type account for set reconcile account
        :return:
        """
        for rec in self:
            if rec.company_id:
                journals = self.env['account.journal'].sudo().search([('company_id', '=', rec.company_id.id)])
                rec.journal_ids = [(6, 0, journals.ids)]
            else:
                rec.journal_ids = [(6, 0, [])]

            reconcile_account_ids = self.env['account.account'].sudo().search([('account_type', '=', 'asset_cash'),
                                                                               (
                                                                               'company_ids', 'in', rec.company_id.id)])
            if reconcile_account_ids:
                rec.reconcile_account_ids = [(6, 0, reconcile_account_ids.ids)]
            else:
