...
class SetuABCSalesAnalysisReport(models.TransientModel):
    ...
    products_ids = fields.Many2many("product.product", 'abc_sale_analysis_report_product_rel', 'abc_sale_analysis_report_id',
                                    'products_id',
                                    string="Products",
    ...
    def _compute_products_ids(self):
        """
            ...
            Purpose: This method is used for return products except combo products
            based on category if user select category from wizard
            otherwise return all products except combo products
        """
        for record in self:
            if record.product_category_ids:
                products = self.env['product.product'].search(
                    [('categ_id', 'child_of', record.product_category_ids.ids), ('product_tmpl_id.type', '!=', 'combo')])
                record.products_ids = products if products else False
            else:
                products = self.env['product.product'].search([('product_tmpl_id.type', '!=', 'combo')])
                record.products_ids = products if products else False
