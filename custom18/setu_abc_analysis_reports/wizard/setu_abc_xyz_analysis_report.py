...
class SetuABCYZAnalysisReport(models.TransientModel):
    ...
    def _compute_products_ids(self):
        """
            ...
            Purpose: This method is used for return storable products
            based on category if user select category from wizard
            otherwise return storable products only
        :return:
        """
        for record in self:
            if record.product_category_ids:
                products = self.env['product.product'].search(
                    [('categ_id', 'child_of', record.product_category_ids.ids),
                     ...
                     ('product_tmpl_id.type', '!=', 'combo')])
                record.products_ids = products if products else False
            else:
                products = self.env['product.product'].search([('product_tmpl_id.is_storable', '=', True),('product_tmpl_id.type', '!=', 'combo')])
                record.products_ids = products if products else False
