...
class SetuInventoryXYZAnalysisReport(models.TransientModel):
    ...
    products_ids = fields.Many2many("product.product", 'inv_xyz_analysis_report_product_rel',
                                    'inv_xyz_analysis_report_id',
                                    'products_id',
                                    string="Products",
    ...
    def _compute_products_ids(self):
        """
            ...
            Purpose: This method is used for return only storable products
            based on category if user select category from wizard
            otherwise return only storable products
        :return:
        """
        for record in self:
            if record.product_category_ids:
                products = self.env['product.product'].search(
                    [('categ_id', 'child_of', record.product_category_ids.ids),('product_tmpl_id.is_storable', '=', True)])
                record.products_ids = products if products else False
            else:
                products = self.env['product.product'].search([('product_tmpl_id.is_storable', '=', True)])
                record.products_ids = products if products else False
