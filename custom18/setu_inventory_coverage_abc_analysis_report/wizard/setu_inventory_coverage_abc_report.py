...
class SetuInventoryCoverageReport(models.TransientModel):
    ...
    def download_report_in_listview(self):
        """
            ...
            Use: inherit method and added name for report header in excel file report
        """
        list_view_id = False
        if self._context.get('is_abc', False):
            list_view_id = self.env.ref(
                'setu_inventory_coverage_abc_analysis_report.setu_inventory_coverage_abc_analysis_bi_report_list_inherit').id
            res = super(SetuInventoryCoverageReport, self.with_context(abc_list_id=list_view_id)).download_report_in_listview()
    ...
    def write_report_data_header(self, workbook, worksheet, row):
        """
            Added By: Shivam Pandya | Date: 21st Oct,2024 | Task: 898
            Use: used to add header for classification column in excel file report
        """
    ...
    def write_data_to_worksheet(self, workbook, worksheet, data, row):
        """
            Added By: Shivam Pandya | Date: 21st Oct,2024 | Task: 898
            Use: inherit method to add values in classification column
        """
