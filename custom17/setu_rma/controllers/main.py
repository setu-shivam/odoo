...
class RmaBackend(CustomerPortal):
    ...
    def return_order_form_submit(self, id=False,access_token=None,  **kw):
        ...
        is_valid = True
        tem = []
        for products in product_list_ids:
            if kw.get('request-qty-' + str(products)) == '' or kw.get('request-qty-' + str(products)) == '0':
                kw['request-qty-' + str(products)] = '0'
                tem.append(0)
        if len(product_list_ids) == len(tem):
            is_valid = False
        if is_valid:
            delivery_order = request.env['stock.picking'].sudo().search([('id', '=', id)])
            ...
            remove_lines = rma_line.sudo().search(
                [('return_order_id', '=', order.id), ('product_id', 'not in', product_list_ids)])
            remove_lines.sudo().unlink()
            for product_list_id in product_list_ids:
                line = rma_line.sudo().search(
                    [('return_order_id', '=', order.id), ('product_id', '=', product_list_id)])
                if kw.get('request-qty-%s' % product_list_id, False) == '' or kw.get('request-qty-%s' % product_list_id,
                                                                                     False) == '0' or kw.get(
                    'add_image-%s' % product_list_id, False) == '':
                    line.unlink()
                else:
                    return_qty_main = int(kw.get('request-qty-%s' % product_list_id, 0))
                    return_flag = False
                    for line_indvidual in line:
                        if line_indvidual.done_qty > return_qty_main:
                            return_qty = return_qty_main
                            return_flag = True
                        else:
                            return_qty = line_indvidual.done_qty
                            return_qty_main = int(
                                kw.get('request-qty-%s' % product_list_id, 0)) - line_indvidual.done_qty

                        line_indvidual.sudo().write({
                            'return_order_reason_id': int(kw.get('return-order-reason-%s' % product_list_id, False)),
            ...
            return request.redirect('/my/return/form/' + kw.get('form_data') + '?access_token=' + kw.get('tokan'))
